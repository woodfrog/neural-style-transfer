...
% size(errpatch)
% size(ones(1, size(errpatch,2)))

original_errpatch = errpatch;

errpatch = [1E10*ones(1, size(errpatch,2)) ; errpatch ; 1E10*ones(1, size(errpatch,2))];

imagesc(transpose(original_errpatch)); axis image;
...
hold on;
for t = 1:size(bestpath,2)
    plot(bestpath(t), t, 'r.', 'LineWidth', 2, 'MarkerSize', 15);
end
hold off;
